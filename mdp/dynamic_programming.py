...
class ValueIteration:
      ...
      def _worker(P, key, flag):
        while True:
          if flag.value > 0:
            ...
            break
      chunksize = np.ceil(self.mdp.states.num_states * self.mdp.actions.num_actions / cpu_count())
      self.workers = []
  ...
  def update(self, parallel=True):
    ...
    value_diff = np.sqrt(np.dot(value_diff, value_diff) / self.mdp.states.num_states)
...
class PolicyIteration:
  ...
  def __init__(self, mdp):
    ...
    self.__I = sp.identity(
      self.mdp.states.num_states,
      dtype=np.float32, format='csr'
    )
    self.__innerloop_maxiter = max(
      int(np.sqrt(self.mdp.states.num_states)),
      100
    )
