...
class NmpcPolar:
  ...
  def __init__(
    ...
    #v = self.wheel_rad * (controls[0] + controls[1]) / 2
    #omega = self.wheel_rad * (controls[1] - controls[0]) / self.wheel_separation
    v = SX.sym('v')
    omega = SX.sym('omega')
  ...
  def solve(self, state, theta, u_prev=None, return_trajectory=False, warm_start=0):
    ...
    if state[0] < self.r_tol:
      sol = self.nmpc_rotation.solve(
        state, theta[1] + theta[2], u_prev=u_prev,
        return_trajectory=return_trajectory, warm_start=warm_start
      )
    ...
    if warm_start==1:
      ...
      for k in range(0, self.N):
        ...
        #if np.abs(calpha) < 1e-3:
        #  calpha = np.sign(calpha) * 1e-3
